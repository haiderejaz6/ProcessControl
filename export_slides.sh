#!/usr/bin/env bash
# Rebuilds the notebook, executes it, exports to reveal.js slides, and
# localizes all external dependencies (reveal.js, MathJax, jquery, require.js)
# so the deck works fully offline in a classroom.
set -euo pipefail
cd "$(dirname "$0")"
...
echo "== Exporting to reveal.js slides (local reveal.js) =="
jupyter nbconvert --to slides "$NB" --output "${SLIDES%.slides.html}" \
    --SlidesExporter.reveal_url_prefix=reveal.js

echo "== Localizing external dependencies =="
python3 - <<'PYEOF'
path = "PSE-823_Lecture-01_Ch1-2_slides.slides.html"
with open(path) as f:
    html = f.read()

# MathJax -> local pruned bundle
old = 'https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.7/latest.js?config=TeX-AMS_CHTML-full,Safe'
new = 'mathjax/MathJax.js?config=TeX-AMS_CHTML-full,Safe'
assert html.count(old) == 1, "MathJax CDN line not found / already replaced"
html = html.replace(old, new)

# Drop jquery/require.js/mermaid entirely -- unused by our content, and
# RequireJS's dynamic module loading is unreliable from a file:// URL anyway
# (see the bootstrap rewrite below).
...
# Replace nbconvert's RequireJS/AMD bootstrap with plain <script src> tags +
# a direct Reveal.initialize() call. RequireJS's dynamic script injection is
# what causes the deck to render as a flat scrollable page instead of an
# actual slideshow when opened directly (file://) in a real browser --
# reveal.js and its notes plugin are both UMD bundles that expose plain
# globals (Reveal, RevealNotes) when loaded as ordinary scripts, so no AMD
...
new_bootstrap = '''<script src="reveal.js/dist/reveal.js"></script>
...
python3 - <<'PYEOF'
path = "PSE-823_Lecture-01_Ch1-2_slides.slides.html"
with open("reveal-lecture-theme.css") as f:
    css = f.read()
with open(path) as f:
    html = f.read()
...
if grep -qE "cdnjs\.cloudflare\.com|unpkg\.com|jsdelivr\.net" "$SLIDES"; then
    echo "WARNING: external CDN references still present!"
    grep -nE "cdnjs\.cloudflare\.com|unpkg\.com|jsdelivr\.net" "$SLIDES"
    exit 1
else
