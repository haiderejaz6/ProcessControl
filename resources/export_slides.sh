...
# served over http.
#
# The canonical notebook is deliberately kept UNEXECUTED (the presenter runs
# it live in RISE), so the static deck is built from a throwaway executed
# copy: code and output both appear on the slides, and nothing is baked into
# the notebook itself.
#
# Run from inside resources/: ./export_slides.sh
set -euo pipefail
cd "$(dirname "$0")"

NB=PSE-823_Python-Libraries-Primer.ipynb
SLIDES=PSE-823_Python-Libraries-Primer_slides.slides.html
COLAB=PSE-823_Python-Libraries-Primer_colab.ipynb
ASSETS=../assets   # shared reveal.js/ + mathjax/, relative to this folder

EXEC=_executed_tmp.ipynb
trap 'rm -f "$EXEC"' EXIT

echo "== Executing a temporary copy =="
jupyter nbconvert --to notebook --execute --output "$EXEC" "$NB"

echo "== Exporting to reveal.js slides (shared repo assets) =="
jupyter nbconvert --to slides "$EXEC" --output "${SLIDES%.slides.html}" \
